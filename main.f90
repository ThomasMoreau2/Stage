program main 
    ...
    real(kind=PR) :: dx, dt, lambda, t_final, Lx, Rx, Mn_i, a, tau, tau_2
    real(kind=PR) :: rho_g, rho_d, u_g, u_d, T_g, T_d, eps, temps
    real(kind=PR) :: v_min, v_max, dv, t1, t2
    real(kind=PR), dimension(:, :), allocatable :: alpha, alpha_np1, beta, max2, max1
    ...
    allocate(max1(0:nx, 0:nv), max2(0:nx, 0:nv))
    ...
    ! Projection de la condition de bord
    do l = 0, nv  
    ...
    do n = 0, nt - 1 
        ...
        do l = 0, nv ! Boucle en vitesse 
            ...
            if (a /= 0.0_PR) then 
                ...
                if (abs(lambda) >= 1.0_PR) then 
                    ...
                    mat_L = make_L(p, lambda, a)
                    mat_M = make_M(p, lambda, a)
                    mat_N = make_N(p, lambda)

                    V_L = make_V_L(p, a)
                    V_M = make_V_M(p, lambda)
                    ...
                    mat_P = make_P(p, lambda, a)
                    mat_Q = make_Q(p, lambda, a)

                    V_O = make_V_O(p, lambda, a)
                    V_P = make_V_P(p, lambda, a)
                    V_Q = make_V_Q(p, a)
                ...
                if (a > 0.0_PR) then ! Cas a positif: on parcourt le vecteur beta de gauche a droite
                    ...
                    if (abs(lambda) >= 1.0_PR) then ! Cas lambda > 1 
                        ...
                        do k = 0, p  ! Boucle pour la serie entiere de exp
                            ...
                            alpha_np1((i-1)*p + 1 : i*p, l) = alpha_np1((i-1)*p + 1 : i*p, l) + (-tau_2 / tau)**k * & 
                                                        matmul(mat_L(k, 1:p, 1:p), beta((i-1)*p + 1 : i*p, l)) + & 
                                                        Mn_i * (-tau_2 / tau)**k * V_L(k, 1:p)

                            beta(i*p + 1 : (i+1)*p, l) = beta(i*p + 1 : (i+1)*p, l) + (-tau_2 / tau)**k * & 
                                                    matmul(mat_M(k, 1:p, 1:p), alpha((i-1)*p + 1 : i*p, l)) + & 
                                                    Mn_i * (-tau_2 / tau)**k * V_M(k, 1:p)

                        end do 

                        beta(i*p + 1 : (i+1)*p, l) = beta(i*p + 1 : (i+1)*p, l) + &
                                                exp(-2.0_PR * tau_2 / tau) * matmul(mat_N, beta((i-1)*p + 1 : i*p, l)) + &
                                                Mn_i * (1.0_PR - exp(-2.0_PR * tau_2 / tau)) * V_N
                        ...
                        alpha_np1((i-1)*p + 1 : i*p, l) = alpha_np1((i-1)*p + 1 : i*p, l) + (-tau_2 / tau)**k * &
                                                    ...
                                                    Mn_i * (-tau_2 / tau)**k * V_P(k, 1:p)

                        beta(i*p + 1 : (i+1)*p, l) = beta(i*p + 1 : (i+1)*p, l) + (-tau_2 / tau)**k * &
                                                matmul(mat_Q(k, 1:p, 1:p), alpha((i-1)*p + 1 : i*p, l)) + & 
                                                Mn_i * (-tau_2 / tau)**k * V_Q(k, 1:p)

                        end do 

                        alpha_np1((i-1)*p + 1 : i*p, l) = alpha_np1((i-1)*p + 1 : i*p, l) + & 
                                                exp(-2.0_PR * tau_2 / tau) *matmul(mat_O, alpha((i-1)*p + 1 : i*p, l)) + & 
                                                Mn_i * (1.0_PR - exp(-2.0_PR * tau_2 / tau)) * V_O
                    ...
                    if (abs(lambda) >= 1.0_PR) then 
                        ...
                        do k = 0, p 
                            ...
                            alpha_np1((j-1)*p + 1 : j*p, l) = alpha_np1((j-1)*p + 1 : j*p, l) + (-tau_2 / tau)**k * & 
                                                            matmul(mat_L(k, 1:p, 1:p), beta(j*p + 1 : (j+1)*p, l)) + & 
                                                            Mn_i * (-tau_2 / tau)**k * V_L(k, 1:p)

                            beta((j-1)*p + 1 : j*p, l) = beta((j-1)*p + 1 : j*p, l) + (-tau_2 / tau)**k * & 
                        ...
                        beta((j-1)*p + 1 : j*p, l) = beta((j-1)*p + 1 : j*p, l) + &
                                                exp(-2.0_PR * tau_2 / tau) * matmul(mat_N, beta(j*p + 1 : (j+1)*p, l)) + &
                                                Mn_i * (1.0_PR - exp(-2.0_PR * tau_2 / tau)) * V_N
                        ...
                        alpha_np1((j-1)*p + 1 : j*p, l) = alpha_np1((j-1)*p + 1 : j*p, l) + & 
                                                exp(-2.0_PR * tau_2 / tau) *matmul(mat_O, alpha((j-1)*p + 1 : j*p, l)) + & 
                                                Mn_i * (1.0_PR - exp(-2.0_PR * tau_2 / tau)) * V_O
                    end if 

                else if (a == 0.0_PR) then

                    alpha_np1((i-1)*p + 1 : i*p, l) = matmul(mat_a_0, alpha((i-1)*p + 1 : i*p, l)) * &
                                                    exp(-2.0_PR * dt / tau) + &
                                                    Mn_i * (1.0_PR - exp(-2.0_PR * dt / tau)) * V_a_0
        ...
        ! print *, MAX(MAXVAL(abs(max2/max1)), 1.0_PR / MINVAL(abs(max2/max1)))
        ! max2 = max1
    ...
    open(unit=13, file='rho_ex.dat', status='replace')
    open(unit=14, file='u_ex.dat',   status='replace')
    open(unit=15, file='T_ex.dat',   status='replace')
    ...
    do i = 0, nx
        ...
        write(13, *) Lx + i*dx, rho_ex(temps, Lx + i*dx, rho_g, rho_d, T_g, T_d)
        write(14, *) Lx + i*dx, u_ex(temps, Lx + i*dx, rho_g, rho_d, T_g, T_d)
        write(15, *) Lx + i*dx, T_ex(temps, Lx + i*dx, rho_g, rho_d, T_g, T_d)
